...
/* positonDecoder
 * Set the Crazyflie Position in the world coordinate system
 */
struct positionPacket_s {
  float x;        // m in the world frame of reference
  float y;        // ...
  float z;        // ...
  float Yaw;
  float aktiv;
...
static void positionDecoder(setpoint_t *setpoint, uint8_t type, const void *data, size_t datalen)
{
  ...
  ASSERT(datalen == sizeof(struct positionPacket_s));
  aktivermode = values->aktiv;
  if (values->aktiv >= 0.5f)
  {
	  aktivermode = 4;
...
LOG_GROUP_START(aktiv)
LOG_ADD(LOG_FLOAT, aktivmode, &aktivermode)
LOG_GROUP_STOP(aktiv)
